...
| `GET /oauth2callback?code=...&state=...` | None | Google's redirect target. Verifies state HMAC + nonce + sub fingerprint, exchanges the code. In single-user mode (allowlist length 1, `MCP_ALLOW_ANY_AUTH0_SUB=false`) persists the encrypted refresh token inline, then 303-redirects to `/oauth/connected` (Post/Redirect/Get, so a browser reload is reload-safe). In multi-user mode (allowlist length > 1, OR `MCP_ALLOW_ANY_AUTH0_SUB=true`) stashes the verified data in `oauth_pending_links` and 303-redirects to `/oauth/confirm`. |
| `GET /oauth/connected` | None | Static, reload-safe success page that the callback (single-user) and the multi-user confirm POST redirect to after a successful link. No query params, no DB, no per-account data. Reloading it is a pure re-render. |
...
| `POST /oauth/confirm` | None | Form-encoded body with `pending_token` + `action=confirm` or `action=cancel`. Confirm consumes the pending row and upserts to `gmail_oauth_tokens` atomically, then 303-redirects to `/oauth/connected` (Post/Redirect/Get, so a reload does not re-POST). Cancel drops the pending row and renders an inline page. Re-checks the allowlist before consuming so a sub removed during the 10-minute window cannot finalize. |
...
4. **Userinfo email check** after the code exchange. Google's `/userinfo` returns the actual logged-in email. The mailbox we persist under is always Google's userinfo email (the source of truth); if it differs from the email the flow was started for, we still persist the row, log a `WARN` with both addresses, and return an HTTP 200 JSON body of the shape `{"status": "connected_with_different_email", "requested": "...", "actual": "..."}` so the connector UI can display the discrepancy. The matching-email path 303-redirects to the static `/oauth/connected` success page (see [Reload-safe success page](#reload-safe-success-page-postredirectget)).

### Reload-safe success page (Post/Redirect/Get)

The callback URL is single-use: the state nonce is consumed atomically on the first hit. The success result used to render inline at that URL, so any reload, prefetch, or duplicate navigation re-ran `consume_nonce`, which then returned nothing, and the page flipped to the "This connection link has already been used or expired" failure page even though the link had genuinely succeeded and the refresh token was already persisted.

The fix is Post/Redirect/Get: on a successful link the callback (single-user) and the multi-user confirm POST `303`-redirect to the static `GET /oauth/connected` page. That page consumes nothing (no code, state, nonce, pending_token, or email) and interpolates no per-account data, so a reload is a pure re-render and the browser's address bar no longer points at the consuming callback URL.

Single-user reload-after-success at the callback itself is also handled: when the nonce is already consumed, the handler checks (read-only) whether a live token already exists for the verified `(auth0_sub, account_email)`. If so it redirects to `/oauth/connected`; otherwise (genuinely expired, never started, or a flow that consumed its nonce but failed before persisting) it returns the real `400` failure page.

Security properties preserved:

- **Replay defense intact.** The reload-after-success branch re-consumes no nonce, re-exchanges no authorization code, and persists no token. The single-use nonce stays consumed and the authorization code is single-use at Google as well. A replay therefore produces no new side effect; only the page shown changes.
- **One-bit signal, acknowledged.** The branch does introduce one observable bit: "a live token exists for this exact `(auth0_sub, account_email)`," surfaced as a `303` versus a `400`. That bit is gated behind possession of a valid HMAC-signed `state` for that exact tuple, which only a principal who completed bearer-authenticated, allowlisted `/oauth/start` can hold. That principal can already read the same fact from `GET /oauth/status`, so no new information is disclosed to anyone who could not already obtain it. The static `/oauth/connected` page itself is account-agnostic and leaks strictly less than the old inline page, which echoed the connected email in its body.
- **Different-email reload residual (by design).** The `connected_with_different_email` path persists under Google's actual email and returns its documented JSON, not a redirect. A reload of that specific callback URL looks up the requested email (the only address carried in `state`) rather than the actual one, finds no token under it, and falls to the `400` failure page. This is an accepted edge for the uncommon "signed in with a different Google account than requested" case; the link itself is already persisted and visible via `/oauth/status`.
...
3. The callback `303`-redirects to `http://localhost:8000/oauth/connected`, a static "Connected" success page that carries no email or token. Reloading that page just re-renders it (the old inline page would have flipped to "already used or expired" on reload). If you signed in with a different Google account than the one you passed as `account_email`, the callback instead returns HTTP 200 JSON of the shape `{"status": "connected_with_different_email", "requested": "you@example.com", "actual": "what-you-actually-signed-in-as@example.com"}` and the row is persisted under `actual` (this path does not redirect). If the response is an HTML error page (state expired, refresh-token-not-issued, code-exchange failure), check the service logs for the specific reason; errors in the user-facing page are intentionally generic to avoid leaking flow internals.
